...
describe("POST /api/users", () => {
  ...
  it("should register new user", async () => {
    const response = await supertest(web).post("/api/users").send({
      username: "primestrider",
      password: "primestrider123",
      name: "prime strider",
    });

    logger.debug(response.body);
    expect(response.status).toBe(200);
    expect(response.body.data.username).toBe("primestrider");
    expect(response.body.data.name).toBe("prime strider");
  });
});
...
describe("POST /api/users/current/update", () => {
  beforeEach(async () => {
    await UserTest.create();
  });
