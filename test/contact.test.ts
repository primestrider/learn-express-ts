...
describe("POST /api/contacts/post/:contactId", () => {
  beforeEach(async () => {
    await UserTest.create();
    await ContactTest.create();
  });
  ...
  it("should be able to update contact", async () => {
    const contact = await ContactTest.get();
    const response = await supertest(web)
      .post(`/api/contacts/update/${contact.id}`)
      .set("X-API-TOKEN", "test")
      .send({
        first_name: "eko",
        last_name: "khannedy",
...
describe("POST /api/contacts/delete/:contactId", () => {
  beforeEach(async () => {
    await UserTest.create();
    await ContactTest.create();
  });
  ...
  it("should be able to remove contact", async () => {
    const contact = await ContactTest.get();
    const response = await supertest(web)
      .post(`/api/contacts/delete/${contact.id}`)
      .set("X-API-TOKEN", "test");
  ...
  it("should reject remove contact if contact is not found", async () => {
    const contact = await ContactTest.get();
    const response = await supertest(web)
      .post(`/api/contacts/delete/${contact.id + 1}`)
      .set("X-API-TOKEN", "test");
...
describe("GET /api/contacts/list", () => {
  ...
  it("should be able to search contact", async () => {
    ...
    expect(response.body.paging.limit).toBe(10);
  });
